...
const BREVO_BLOCKED_CONTACTS_PAGE_SIZE = 100
...
function describeSkippedStep(reason) {
  switch (reason) {
    ...
    case 'blocked_contact':
      return 'recipient email is blocked in Brevo'
    default:
...
async function brevoRequest(path, { method = 'POST', body } = {}) {
...
async function findBrevoBlockedContact(email) {
  ...
  while (true) {
    const page = await brevoRequest(
      `/smtp/blockedContacts?limit=${BREVO_BLOCKED_CONTACTS_PAGE_SIZE}&offset=${offset}`,
      { method: 'GET' }
    )
    ...
    const match = contacts.find(
      (contact) => normalizeValue(contact.email).toLowerCase() === normalizedEmail
    )
    ...
    if (contacts.length === 0 || offset >= (page?.count ?? 0)) {
      return null
    }
  }
}
...
export async function sendAutoReplyEmail({ email, firstName, formName }) {
  ...
  const blockedContact = await findBrevoBlockedContact(email)

  if (blockedContact?.skipped) {
  ...
  if (blockedContact) {
    return {
      ...
      detail:
        blockedContact.reason?.message ||
        'Recipient email is blocked in Brevo due to an earlier delivery failure.',
    }
  }
...
export function assertNotSkipped(result, label) {
  if (result?.skipped) {
    throw new Error(
      `${label} skipped: ${result.detail || describeSkippedStep(result.reason)}`
    )
  }
...
export function logOptionalStepResult(context, result) {
  if (result?.skipped) {
    console.warn(`${context} skipped`, {
      ...
      detail: result.detail || describeSkippedStep(result.reason),
    })
    return
  }

  if (result) {
    console.info(`${context} succeeded`, {
      id: result.id ?? null,
...
export function isBlockedContactError(error) {
  return (
    error instanceof Error &&
    error.message.includes('recipient email is blocked in Brevo')
  )
}
